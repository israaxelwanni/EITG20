...
class dispatcher(larger):
    ...
    def randomDispatch(self):
        # Here code should be added, exercise 5.2
...
queues = [queue(MU,s) for MU in [1, 0.5, 1.5, 1, 1]] # change for exercise 5.7
...
## exercise 5.3: 
# the sum of the mean nbr of customers in all the queuing systems 
# from the queuing theorem
rho = 0.8/1 # lambda / mu
L = rho / (1 - rho) * 5 # 5 queuing systems in series, so multiply by 5

print(f"Expected E[N] for the system: {L}")

# exercise 5.4: the actual total sum of mean nbr of customers in the system from the simulation
total_E_N = sum(q.sumMeasurements / q.numberOfMeasurements for q in queues)
print(f"Total E[N] for the system: {total_E_N}")

# exercise 5.5: mean total time in the system
# is already given.
